...
>以面向对象的思路来看待：
>
>- 类是对象的模板，在C中，使用结构体来充当类。
>  - 结构体在头文件中声明。
>  - 类的方法，在C中，则使用函数指针，结构体中存储着函数的指针即可。
>  - 权限控制通过static和头文件来控制，头文件声明对外开放的接口，不对外开放的在`.c`文件里使用static声明并实现。
>  - 结构体定义在`.c`文件里，并实现对外开放的接口函数用于外部获取到定义好的结构体。
>- 一个对象即一个模块，当然这个对象有可能依赖于其他对象。
