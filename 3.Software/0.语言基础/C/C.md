...
>这个阶段主要分为三部分：
>
>1. 自然语言到编程语言的翻译和转化：将现实世界的特定场景下的特定需求用文字描述出来，然后尝试用编程语言去解决、去实现需求。
>   - 语法。
>   - 数据结构（根据特点，结合需求去选择用什么容器将数据存储起来。）
>   - 逻辑控制。
>2. 类（对象）的设计。（代码优化）
>3. 方法的设计。（代码优化）
>
>一是语言工具的学习，熟练使用一门编程语言。（语言基本知识，多写多用就会记得牢固了。至于如何去用是个值得思考的问题）
>
>二是熟悉一些常用的数据结构的优缺点。
>
>三是锻炼自己从需求到实现需求的这个能力。
...
>这个阶段着重提高这几方面的能力：
>
>1. 广度拓展：项目程序中有什么，都是干啥的，优缺点与应用场景。
>2. 可维护性，可拓展性。
>3. 模块拆分，模块解耦。
>
>### ③主导设计：
...
>
>作者：黑客宾
>链接：https://www.zhihu.com/question/61976638/answer/197329404
>来源：知乎
...
const int NUM1 = 99;
int main(){
    /* 局部作用域下使用const声明的变量可以通过指针进行修改内容 */
	const int NUM2 = 99;
    int* b = &NUM2;
    printf("%d\n", NUM2);
    *b = 33;
    printf("%d\n", NUM2);
    /* 如果局部变量同时使用static和const，那也不能被修改 */
...
正数的原码、补码、反码一致，负数的反码为除符号位其余的取反、补码为反码末位加1。计算机中数
...
int main(void){
    ...
    /* 输出8，其实就是存储指针变量的内存的大小，无法计算出指向的内存的大小 */
    printf("%d\n",  sizeof (s)); // 8
    return 0;
}
```
...
位运算符：&、|、~、^（异或，异则真）、<<（左移，高位移掉，低位补0）、>>（右移，对于无符号数是逻辑右移，高位都补0，移出去的位都丢弃；对于有符号数是算术右移，会保持符号位不变，并且符号位是啥高位就补啥，移出去的位都丢弃）。
...
>[C语言变长数组讲解_猴子居士的博客-CSDN博客](https://blog.csdn.net/houzijushi/article/details/80245894)
>
...
- 使用static声明的全局变量，该变量的作用域就受限于该翻译单元，对其它翻译单元不可见。如果 static 用于块作用域声明，那么变量的作用域则受限于该块。块作用域的静态变量无链接，只能在该快内使用； 文件作用域的变量使用了static声明则只有内部链接（没了外部链接），仅在该文件内使用。
...
>     * 8 + 4 + 1 = 13,因为最后结果要是最大成员的整数倍，所以取8 + 4 + 4 = 16
>     * */
>    printf("Test1: %d\n", sizeof(test1));
...
>```
>
>2、对于包含数组和指针的结构体，可以用和上面相同的方法进行计算，一般指针的大小都是固定的4个字节（32位平台下是4自己，64位平台下是8字节），因为不管什么类型的指针只需要储存地址，不需要储存地址指向空间的内容。
>
>```c
>/* 含指针的，64平台下，以下都为24字节 */
>struct Test1
